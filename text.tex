...
\textsc{\underbar{Unabhängigkeit}} \\
    ...
    Wir sagen \((X_i)_{i \in I} \sim \mathcal D\) i.i.d. für eine Verteilung \(\mathcal D\), wenn wir meinen, dass alle \(X_i\) unabhängig aber gleich verteilt sind. \pagebreak \\
...
\textsc{\underbar {Diverse Verteilungen}} \\
    ...
    \(\rho \QDp \R \longrightarrow \R_{\ge 0}, t \mapsto \lambda e^{-\lambda t}\), wenn \(t \ge 0\) und sonst \(0\). Es gilt: \(\Ew(\Exp(\lambda)) = \lambda^{-1}\) und \(\Var(\Exp(\lambda)) = \lambda^{-2}\). CDF: \(t \mapsto 1 - e^{\smi \lambda t}\) \\
...
\subsubsubsection{Eigentschaften der Normalverteilung}{
    Sei \(X \sim \Norm(\mu, \sigma^2)\) und \(\delta \in \R\), \(\lambda \ne 0\). Dann gilt: \(\lambda X + \delta \sim \Norm(\lambda\mu + \delta, \lambda^2\sigma^2)\), insbesondere \(\frac{X-\mu}{\sigma} \sim \Norm(0,1)\). Es gilt auch: \(\Ew(X) = \mu\) und \(\Var(X) = \sigma^2\).
...
\subsubsubsection{Konvergenz in Verteilung und Zentraler Grenzwertsatz}{
    \textit{Konvergenz in Verteilung}: Sei \((X_n)_n\) beliebige Folge von ZV, wir sagen \((X_n)_n\) konvergiert in Verteilung gegen \(\mathcal D\), wenn gilt:\\
    \(\forall u \in \R, \text{ sodass } F_{\mathcal D} \text{ stetig in } u\QDp \lim_{n}F_{X_n}(u) = F_{\mathcal D}(u)\) \\
    \textit{Zentraler Grenzwertsatz}: Seien \(\mu \in \R, v > 0, (X_i)_{i \ge 1}\) Folge von i.i.d ZV über dem selben W'raum mit \(\Ew(X_i) = \mu\) und \(\Var(X_i) = v\) für alle \(i \ge 1\). Dann konvergiert \(\left(\frac{1}{\sqrt{n}}\sum_{i = 1}^{n}\frac{X_i - \mu}{\sqrt{v}}\right)_{n \ge 1}\) in Verteilung gegen \(\Norm(0,1)\).
}
\subsubsection*{Konfidenzintervalle in Normalverteilungsmodellen}
\subsubsubsection{Statistisches Modell}{
    Sei \(n,d \in \N_1, \Theta \subseteq \R^d\). Ein statistisches Modell ist eine Familie \((D_i^\vartheta)_{(\vartheta, i){\in}\Theta\times[n]}\) von Verteilungen für eine ZV.\\
    Nach Existenzsatz existiert für jeden Parameter \(\vartheta \in \Theta\) eine Familie \((X_i^\vartheta)_{i \in [n]}\) von ZV mit folgenden Eigenschaften:\\
    \(X^\vartheta\) ist unabhängig und \(\forall i \in [n]\QDp X_i^\vartheta \sim \D_i^\vartheta\). Um nicht immer den Existenzsatz anzuwenden schreiben wir \((X \sim \D^\vartheta_i)_{(\vartheta, i)\in\Theta\times[n]}\) um direkt sowohl die Familie der Verteilungen als auch die existierende Familie an unabhängigen ZV zu definieren. \\
    Bei Modellen mit endlichen oder kleiner Anzahl an Verteilungen pro Paramenter \(\vartheta\) schreiben wir auch gerne Beispielweise \((X = (X_1 \dots X_n) \sim \D)_{\vartheta {\in} \Theta}\) oder \(\left(\begin{smallmatrix}
        X = (X_1, \dots, X_n) \sim \D_1^\vartheta \\
        Y = (Y_1, \dots, Y_n) \sim \D_2^\vartheta
    \end{smallmatrix}\right)_{\vartheta \in \Theta}\) für die Verteilungen \(\D^\vartheta\) bzw. \(\D_1^\vartheta, \D_2^\vartheta\).\\
}
\subsubsubsection{Reele Kenngröße}{
    Sei \((X \sim \D_i^\vartheta)_{(\vartheta, i)\in\Theta\times[n]}\) ein statistisches Modell und \(\tau \QDp \Theta \longrightarrow \R\). Wir nennen \(\tau\) eine reelle Kenngröße, wenn wir mittels \(\tau\) Parameter auf eine bestimmte Größe abbilden wollen. Zum Beispiel wäre \(\vartheta \mapsto \frac{b-a}{2}\) eine reele Kenngröße für uniform verteilte Modelle, wenn \((a,b) = \vartheta\) gilt.
}
\textsc{\underbar{Motivation}}\\
    Sei \(u \in \R^n\) eine Stichprobe, dann wollen wir ein statistisches Modell \((X \sim \D_i^\vartheta)_{(\vartheta, i)\in\Theta\times[n]}\) so wählen, dass es ein \(\vartheta^*\) gibt, den sogenannten \textquote{wahren Parameter}, sodass \(u\) durch einen Prozess entstanden ist, der gut modelliert wird durch \(X^{\vartheta^*}\). Man würde gerne \(\vartheta^*\) und dann auch \(\tau(\vartheta^*)\) approximieren.
\subsubsection*{Punktschätzung}
\subsubsubsection{Punktschätzer}{
    Sei \((X \sim \D_i^\vartheta)_{(\vartheta, i)\in\Theta\times[n]}\) statistisches Modell und \(\tau\QDp \Theta \longrightarrow \R\) eine reelle Kenngröße. Ein Punktschätzer für \(\tau\) ist schlicht eine Funktion \(T\QDp \R^n \longrightarrow \R\). Um möglichst effiziente Punktschätzer zu konstruieren, werden wir einige Eigenschaften definieren.
}
\subsubsubsection{Erwartungstreue}{
    Für alle \(\vartheta \in \Theta\) definieren wir den Bias von \(T\) für \(\tau\) als \(\B^{\vartheta, \tau}(T(X)) := \Ew^\vartheta(T(X)) - \tau(\vartheta)\), wenn der EW existiert. Wir nennen \(T\) einen erwartungstreuen oder unbiased Punktschätzer für \(\tau\), wenn der Bias stets \(0\) ist, also gilt: \(\forall \vartheta \in \Theta \QDp \Ew^\vartheta(T(X)) = \tau(\vartheta)\)
}
\subsubsubsection{Erwartungstreue Schätzer für EW und Varianz}{
    Sei \(n \ge 2\) und seien \(\mu \QDp \Theta \longrightarrow \R\) und \(v \QDp \Theta \longrightarrow \R_{\ge 0}\) reele Kenngrößen, sodass \(\mu(\vartheta) = \Ew(\D^\vartheta)\) und \(v(\vartheta) = \Var(\D^\vartheta)\) für alle \(\vartheta \in \Theta\)\\
    Dann sind mit \(\langle \cdot \rangle\QDp \R^n \longrightarrow \R, u \mapsto \frac{1}{n}\sum_{i=1}^{n}u_i\) die Funktionen \(\langle \cdot \rangle\) und \(s^2\QDp \R^n \longrightarrow \R_{\ge 0}, u \mapsto \frac{1}{n-1}\sum_{i=1}^{n}(u_i - \langle u \rangle)^2\) jeweils erwartungstreue Schätzer für \(\mu\) (also \(\langle \cdot \rangle\)) bzw. \(v\) (also \(s^2\)). \\
    Sei \(u \in \R^n\) Stichprobe, dann nennen wir \(\langle u \rangle\) den empirischen Erwartungswert von \(u\) und \(s^2(u)\) die empirische Varianz von \(u\).
}
