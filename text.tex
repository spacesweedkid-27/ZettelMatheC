...
\subsubsubsection{Erwartungswerte von Verteilungen}{
    Es gilt: \(\Ew(\Ber(p)) = p\), \(\Ew(\Bin(n,p)) = n\cdot p\) und \(\Ew(\Geo(p)) = \frac{1-p}{p}\). Des Weiteren gilt: \(|\Ew(X)| \le \Ew(|X|)\)
}
...
\subsubsubsection{Varianz}{
    ...
    Es gilt: \(\Var(\Ber(p)) = p(1-p)\), \(\Var(\Bin(n,p)) = np(1-p)\) und \(\Var(\Geo(p)) = \frac{1-p}{p^2}\)\\
